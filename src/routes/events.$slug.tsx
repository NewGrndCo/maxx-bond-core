...
function EventDetailPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { ...event, image_url: await signStorageUrl(event.image_url) };
    },
  });
  if (isLoading)
    return (
  ...
  return (
    <main className="detail-page section-shell">
      <Link to="/">← Back to Maxx Bond</Link>
      <article className="detail-card glass">
        {data.image_url && <img src={data.image_url} alt={`${data.title} artwork`} />}
        <div>
          ...
          {data.event_time && (
            <p>
              <Clock /> {data.event_time.slice(0, 5)}
            </p>
          )}
          <p>
            <MapPin /> {data.venue} · {data.city}
          </p>
          <div className="detail-description">{data.description}</div>
          <div className="detail-actions">
            {data.ticket_url && (
              <a className="listen-button" href={data.ticket_url} target="_blank" rel="noreferrer">
                Tickets
              </a>
            )}
            {data.additional_url && (
              <a
                ...
                href={data.additional_url}
                target="_blank"
                rel="noreferrer"
              >
                {data.cta_label || "More Info"}
              </a>
            )}
          </div>
        </div>
